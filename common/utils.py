...
# sample one neigh of "size" nodes from one random graph of the graphs, 
# bad implementation and conception: but it works generally
...
def sample_neigh_from_nodes(graphs, size, start_nodes):
    ...
    while True:
        ...
        #graph = random.choice(graphs)
        graph = graphs[idx]
        start_node = random.choice(start_nodes[idx])
        neigh = [start_node]
        frontier = list(set(graph.neighbors(start_node)) - set(neigh))
        visited = set([start_node])
        while len(neigh) < size and frontier:
...
def sample_neigh_from_node(start_node, graph, size):
    neigh = [start_node]
    frontier = list(set(graph.neighbors(start_node)) - set(neigh))
    visited = set([start_node])
    while len(neigh) < size and frontier:
